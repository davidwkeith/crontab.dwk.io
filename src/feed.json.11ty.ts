...
export default class JSONFeed {
  ...
  async render(data: {
    site: { title: string; url: URL; description: string };
    schema: { author?: { name?: string; url?: string; image?: string } };
    ...
    const feed = {
      ...
      icon: new URL('/img/favicons/favicon-512.png', data.site.url),
      author: {
    ...
    for (const item of data.collections['post']) {
      ...
      const imgURL = item.data.hero?.src
        ? (await Image(item.data.hero.src, imgConfig)).webp[0]?.url
        : undefined;
      const bannerImgURL = item.data.banner?.src
        ? (await Image(item.data.banner.src, imgConfig)).webp[0]?.url
        : undefined;
      ...
      feed.items.push({
        ...
        url,
        content_html:
          item.templateContent ?? item.data.content ?? 'ERROR: No content',
        ...
        banner_image: bannerImgURL
          ? new URL(bannerImgURL, data.site.url)
          : undefined,
